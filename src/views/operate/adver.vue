...
export default{
  name: 'PagePermission',
    ...
    return {
      ...
      total:0,
      listUrl:'/api/advertisingAdmin/findAdvertisingInfoList',
    	delUrl:'/api/advertisingAdmin/delAdvertisingInfo',
    }
  },
  methods: {
...
formatTime(row){
  ...
  upformatTime(row){
    ...
    update(row){
      this.$router.push({path:'/operate/addAdver',query:{id:row.id,placeId:this.pagination.placeId}});
    },
    add(){
    	 this.$router.push({path:'/operate/addAdver',query:{placeId:this.pagination.placeId}});
    },
  },
   created() {
   	if(this.$route.query!=null){
   	  	this.pagination.placeId=this.$route.query.placeId;
   	    this.getList();
   	 }
  },
}
